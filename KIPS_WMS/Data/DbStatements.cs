namespace KIPS_WMS.Data
{
    public static class DbStatements
    {
        ...
        public const string FindCustomersStatement = "SELECT * FROM Customers Where Customer_barcode = '{0}' OR Customer_code LIKE '%{0}%' OR Customer_name LIKE '%{0}%'";
        public const string FindItemsStatement = "SELECT * FROM Items Where Item_barcode = '{0}' OR Item_code LIKE '%{0}%' OR Item_description LIKE '%{0}%'";
    }
}
